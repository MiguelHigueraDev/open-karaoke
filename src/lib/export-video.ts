...
export async function exportToVideo({
  ...
  // Set up audio destination for recording
  const dest = audioCtx.createMediaStreamDestination();
  const source = audioCtx.createBufferSource();
  source.buffer = audioBuffer;
  source.connect(dest);
  // Don't connect to audioCtx.destination — no audible playback during export
  ...
  return new Promise<Blob>((resolve, reject) => {
    ...
    function renderFrame() {
      ...
      // Use audio context time for sync
      ...
      if (currentTime < duration) {
        ...
        // Give a short delay then stop recording
        setTimeout(() => {
          recorder.stop();
          source.stop();
        }, 200);
...
export function drawFrame(
  ...
  // Compute visible window (same logic as karaoke-renderer)
...
function drawWordModeText(
  ...
  for (let i = 0; i < line.words.length; i++) {
    ...
    // Settle offset (matching karaoke-renderer)
    const maxOffset = 4;
    ...
    if (wordProgress > 0 && wordProgress < 1) {
      // Partial: draw highlighted part, then dim part
      const splitX = x + wordWidth * wordProgress;

      // Clip height: generous padding around baseline-middle text
      const clipTop = wordY - fontSize;
      const clipHeight = fontSize * 2;

      // Highlighted portion (clip left)
      ctx.save();
      ctx.beginPath();
      ...
      // Dim portion (clip right)
      ctx.save();
      ctx.beginPath();
      ctx.rect(splitX, clipTop, wordWidth - wordWidth * wordProgress + 1, clipHeight);
      ctx.clip();
      ctx.fillStyle = COLOR_TEXT_DIM;
      ctx.fillText(word.text, x, wordY);
      ctx.restore();
...
function drawInstrumentalDots(
  ...
  // Scale (matching instrumental-dots.tsx)
